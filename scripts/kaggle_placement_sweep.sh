...
# Recompose/rerender several foreground placements on Kaggle and build a contact
# sheet from the first rendered test image of each variant. This reuses existing
# trained models, so after t4full has completed it should not retrain.

PROJECT_DIR="${PROJECT_DIR:-/kaggle/working/CMU-HSL}"
MODE="${MODE:-t4full}"
LOG_DIR="${LOG_DIR:-/kaggle/working}"
MONTAGE_PATH="${MONTAGE_PATH:-/kaggle/working/hsl_outputs/placement_sweep_montage.png}"
...
run_variant() {
  ...
  bash scripts/kaggle_run_all.sh "$MODE"
}
...
montage_path = Path(os.environ.get("MONTAGE_PATH", "/kaggle/working/hsl_outputs/placement_sweep_montage.png"))
tags = [
    "tiny_soft",
    "soft_xleft",
    "soft_xright",
...
tiles = []
for tag in tags:
    pattern = f"/kaggle/working/outputs/render_composed_bicycle_chair_t4full_{tag}/test/ours_30000/renders/*.png"
    imgs = sorted(glob.glob(pattern))
    if not imgs:
        pattern = f"/kaggle/working/outputs/render_composed_bicycle_chair_t4full_{tag}/**/renders/*.png"
